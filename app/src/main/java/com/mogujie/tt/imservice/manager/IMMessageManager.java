...
public class IMMessageManager extends IMManager{
        ...
        imSocketManager.sendRequest(msgData,sid,cid,new Packetlistener(getTimeoutTolerance(messageEntity)) {
            ...
            public void onSuccess(Object response) {
                try {
                    ...
                    //当消息发送成功后，假如此消息为红包收到的响应消息，则不需要再存数据库，之前已经将
                    //展示的内容存入过，此时如果再存入实际发送的内容，则可能发生响应消息不对的问题
                    if(messageEntity instanceof HongbaoAckMessage){
                        messageEntity.setContent(((HongbaoAckMessage)messageEntity).getShowContent());
                    }
                    /**主键ID已经存在，直接替换*/
    ...
    public void onRecvMessage(IMMessage.IMMsgData imMsgData) {
        ...
        logger.i("chat#onRecvMessage,recvMessage的showDisplayType:"+recvMessage.getDisplayType()+",showMsgType:"+recvMessage.getMsgType());
    ...
    public void sendHongbao(HongbaoMessage hongbaoMessage) {
        logger.i("chat#text#hongbaoMessage");
        hongbaoMessage.setStatus(MessageConstant.MSG_SENDING);
        long pkId =  DBInterface.instance().insertOrUpdateMessage(hongbaoMessage);
        sessionManager.updateSession(hongbaoMessage);
        sendMessage(hongbaoMessage);
    }
    public void sendHongbaoAck(HongbaoAckMessage hongbaoAckMessage) {
        logger.i("chat#text#hongbaoAckMessage");
        hongbaoAckMessage.setStatus(MessageConstant.MSG_SENDING);
        sendMessage(hongbaoAckMessage);
    }
