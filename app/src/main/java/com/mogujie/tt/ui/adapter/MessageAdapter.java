...
import com.mogujie.tt.ui.activity.HongbaoOpenActivity;
...
import com.mogujie.tt.ui.widget.message.HongbaoAckRenderView;
        ...
        try {
            ...
            if (obj instanceof Integer) {
                ...
                switch (info.getDisplayType()) {
                    ...
                    case DBConstant.SHOW_HONGBAO_TYPE:
                        type = isMine ? RenderType.MESSAGE_TYPE_MINE_HONGBAO
                                : RenderType.MESSAGE_TYPE_OTHER_HONGBAO;
                        break;
                    case DBConstant.SHOW_HONGBAO_ACK_TYPE:
                        type = isMine ? RenderType.MESSAGE_TYPE_MINE_HONGBAOACK
                                : RenderType.MESSAGE_TYPE_OTHER_HONGBAOACK;
                        break;
                    default:
                        break;
                }
...
//                imageMessage.setPath(loaclPath);//下载的本地路径不再存储
     ...
     * @return
     */
    private View hongbaoMsgRender(final int position, View convertView, final ViewGroup viewGroup, final boolean isMine) {
        HongbaoRenderView hongbaoRenderView;
        final HongbaoMessage hongbaoMessage = (HongbaoMessage) msgObjectList.get(position);
        UserEntity userEntity = imService.getContactManager().findContact(hongbaoMessage.getFromId());

        if (null == convertView) {
            hongbaoRenderView = HongbaoRenderView.inflater(ctx, viewGroup, isMine); //new TextRenderView(ctx,viewGroup,isMine);
        } else {
            hongbaoRenderView = (HongbaoRenderView) convertView;
        }

        hongbaoRenderView.findViewById(R.id.message_ll).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(ctx, HongbaoOpenActivity.class);
                boolean hongbaoStealedOut = false;

                //Todo,联网检查红包是否还有剩余余额未被领取
                checkBalanceEnough(hongbaoStealedOut);

                String fromName = "me";
                if(loginUser.getPeerId() == hongbaoMessage.getFromId()){
                    Toast.makeText(ctx, "此红包是我本人发的红包", Toast.LENGTH_SHORT).show();
                }else{
                    Toast.makeText(ctx, "此红包不是我本人发的，是" + hongbaoMessage.getFromId() + "发的", Toast.LENGTH_SHORT).show();
                    UserEntity userEntity = imService.getContactManager().findContact(hongbaoMessage.getFromId());
                    fromName = userEntity.getRealName();
                }
                intent.putExtra("hongbaoStealedOut", hongbaoStealedOut);
                intent.putExtra("fromName", fromName);
                intent.putExtra("toName", loginUser.getMainName());
                ctx.startActivity(intent);
            }
        });

        final TextView textView = hongbaoRenderView.getMessageContent();

        // 失败事件添加
        hongbaoRenderView.getMessageFailed().setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View arg0) {
                MessageOperatePopup popup = getPopMenu(viewGroup, new OperateItemClickListener(hongbaoMessage, position));
                popup.show(textView, DBConstant.SHOW_ORIGIN_TEXT_TYPE, true, isMine);
            }
        });
        ...
        textView.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                // 弹窗类型
                MessageOperatePopup popup = getPopMenu(viewGroup, new OperateItemClickListener(hongbaoMessage, position));
                boolean bResend = hongbaoMessage.getStatus() == MessageConstant.MSG_FAILURE;
                popup.show(textView, DBConstant.SHOW_ORIGIN_TEXT_TYPE, bResend, isMine);
                return true;
            }
        });
        ...
        // url 路径可以设定 跳转哦哦
        final String content = hongbaoMessage.getContent();
        textView.setOnTouchListener(new OnDoubleClickListener() {
            @Override
            public void onClick(View view) {
                //todo
        ...
        hongbaoRenderView.render(hongbaoMessage, userEntity, ctx);
        return hongbaoRenderView;
    }


    /**
     * hongbaoAck类型的: 1. 设定内容Emoparser
     * 2. 点击事件  单击跳转、 双击方法、长按pop menu
     * 点击头像的事件 跳转
     *
     * @param position
     ...
     * @return
     */
    private View hongbaoMsgAckRender(final int position, View convertView, final ViewGroup viewGroup, final boolean isMine) {
        HongbaoAckRenderView hongbaoAckRenderView;
        final HongbaoAckMessage hongbaoAckMessage = (HongbaoAckMessage) msgObjectList.get(position);
        UserEntity userEntity = imService.getContactManager().findContact(hongbaoAckMessage.getFromId());

        if (null == convertView) {
            hongbaoAckRenderView = HongbaoAckRenderView.inflater(ctx, viewGroup, isMine); //new TextRenderView(ctx,viewGroup,isMine);
        } else {
            hongbaoAckRenderView = (HongbaoAckRenderView) convertView;
        }

        final TextView textView = hongbaoAckRenderView.getMessageContent();

        hongbaoAckRenderView.render(hongbaoAckMessage, userEntity, ctx);
        return hongbaoAckRenderView;
    }

    private void checkBalanceEnough(boolean hongbaoStealedOut) {
        if(hongbaoStealedOut){
        ...
        try {
            ...
            switch (renderType) {
                ...
                case MESSAGE_TYPE_MINE_HONGBAO:
                    convertView = hongbaoMsgRender(position, convertView, parent, true);
                    break;
                case MESSAGE_TYPE_OTHER_HONGBAO:
                    convertView = hongbaoMsgRender(position, convertView, parent, false);
                    break;
                case MESSAGE_TYPE_MINE_HONGBAOACK:
                    convertView = hongbaoMsgAckRender(position, convertView, parent, true);
                    break;
                case MESSAGE_TYPE_OTHER_HONGBAOACK:
                    convertView = hongbaoMsgAckRender(position, convertView, parent, false);
                    break;
            }
            return convertView;
        } catch (Exception e) {
